...
/* Move tha ball down when the button is clicked */
...
/* Get the number. If it is not a number or it is negative, 
show an eorror in the error. Otherwise, first show the percent 
out of 10,000 towards goal. Next, show a visual representation with a box and a gradient */
...
document.getElementById("btn-donate").onclick = () =>{
    ...
    let error = document.getElementById("error-donation");
    const goal = 10000;
    error.innerHTML = "";
    if(input < 1){
        error.innerHTML = "Invalid Number.";
    }
    const percent = txt-donations / goal * 100;
    console.log(percent + "%");
}